...
from dataset_generate import ROI_pool_feature
from sp_model_Net import SPNet
...
if(TRAIN_ON):
  ...
  rgb_pretrain_dataset=ROI_pool_feature(STATE_TRTE,dataset_type)
  ...
  train_dataset = ROI_pool_feature(STATE_TRTE,dataset_type)
...
if(TEST_ON):
  ...
  for dataset_type in range(1,4): #dataset_type=1 #SET: 1~3
    ...
    test_dataset = SPSalientObjDataset(STATE_TRTE,dataset_type,debug=DEBUG,sp_update=False,create=False) 
    ...
    for i, data_record in enumerate(test_loader):
          ...
          # visualization
          result_val_2d=visualize_results_sp1d(STATE_TRTE, Ver_Train, epoch, (im_size[0]), sig_results, rgb_path, gt_path, dep_path, sp_seg)   
